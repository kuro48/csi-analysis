...
async def queue_breathing_analysis(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"タスクのキューへの追加に失敗しました: {str(e)}"
        )
    ...
    try:
        if len(csi_data_ids) == 0:
            raise HTTPException(
                ...
                detail="CSIデータIDリストが空です"
            )

        if len(csi_data_ids) > 100:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="一度に処理できるCSIデータは100件までです"
            )
...
async def get_task_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"タスクステータス取得に失敗しました: {str(e)}"
        )
...
async def get_queue_stats(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"キュー統計情報取得に失敗しました: {str(e)}"
        )


@router.post("/maintenance")
async def queue_maintenance_task(
    maintenance_type: str = Body(..., description="メンテナンス種別"),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if maintenance_type not in valid_types:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"無効なメンテナンス種別です。有効な値: {valid_types}"
            )
...
async def start_workers(
    num_workers: int = Body(default=3, description="ワーカー数"),
    current_user: User = Depends(get_current_user)
):
    """
    タスクワーカー開始（管理者のみ）
