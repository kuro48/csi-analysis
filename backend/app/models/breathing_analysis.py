...
class BreathingAnalysis(BaseModel):
    ...
    # 関連データ
    csi_data_id = Column(BaseModel.id.type, ForeignKey("csi_data.id", ondelete="CASCADE"), nullable=False, index=True)
    device_id = Column(BaseModel.id.type, ForeignKey("devices.id", ondelete="CASCADE"), nullable=False, index=True)
...
class Alert(BaseModel):
    ...
    acknowledged_by = Column(
        BaseModel.id.type, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True  # 確認者検索用
    )
