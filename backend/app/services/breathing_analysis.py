...
from app.models.breathing_analysis import Alert, BreathingAnalysis
...
class BreathingAnalysisService:
    ...
    async def create_analysis_result(
        ...
        # CSIデータの存在確認と所有者チェック
        csi_data = (
            db.query(CSIData).join(Device).filter(and_(CSIData.id == csi_data_id, Device.owner_id == user_id)).first()
        )
        ...
        if device:
            ...
            # 異常値検出とアラート生成
            await BreathingAnalysisService._check_anomalies(db, breathing_analysis, device.device_id)
                    ...
                    if zkp_service:
                        ...
                        asyncio.create_task(
                            BreathingAnalysisService._generate_zkp_async(
                                zkp_service, analysis_data, breathing_analysis.id
                            )
                        )
                    else:
    ...
    @staticmethod
    async def _generate_zkp_async(zkp_service, analysis_data: BreathingAnalysisResult, analysis_id: uuid.UUID):
        """
        ZKP証明の非同期生成
    ...
    @staticmethod
    async def _check_anomalies(db: Session, analysis: BreathingAnalysis, device_id: str):
        """呼吸異常検出とアラート生成"""
        alerts_to_create = []
    ...
    def get_analysis_list(
        db: Session, user_id: uuid.UUID, filters: BreathingAnalysisFilter = None, page: int = 1, page_size: int = 50
    ...
    @staticmethod
    def get_latest_analysis(db: Session, device_id: str, user_id: uuid.UUID) -> Optional[BreathingAnalysis]:
        """最新の呼吸解析結果取得"""
        return (
            db.query(BreathingAnalysis)
            .join(Device)
            .filter(and_(Device.device_id == device_id, Device.owner_id == user_id))
            .order_by(desc(BreathingAnalysis.analysis_timestamp))
            .first()
        )

    @staticmethod
    def get_device_analysis_stats(
        db: Session, device_id: str, user_id: uuid.UUID, days: int = 7
    ) -> BreathingAnalysisStats:
        """デバイスの呼吸解析統計情報取得（最適化版）"""

        device = db.query(Device).filter(and_(Device.device_id == device_id, Device.owner_id == user_id)).first()
        ...
        stats_query = (
            ...
            .filter(and_(BreathingAnalysis.device_id == device.id, BreathingAnalysis.analysis_timestamp >= start_date))
            .first()
        )
        ...
        # 最新解析日時（別途取得）
        latest_analysis = (
            db.query(func.max(BreathingAnalysis.analysis_timestamp))
            .filter(and_(BreathingAnalysis.device_id == device.id, BreathingAnalysis.analysis_timestamp >= start_date))
            .scalar()
        )
    ...
    def get_breathing_trends(
        ...
        analyses = (
            ...
            .filter(and_(BreathingAnalysis.device_id == device.id, BreathingAnalysis.analysis_timestamp >= start_time))
            .order_by(BreathingAnalysis.analysis_timestamp)
            .all()
        )
        ...
        for analysis in analyses:
            ...
            breathing_rate_data.append({"timestamp": timestamp_str, "value": analysis.breathing_rate})

            confidence_data.append({"timestamp": timestamp_str, "value": analysis.confidence_score})
        ...
        result = {
            ...
            "summary": {
                "avg_breathing_rate": (
                    sum([a.breathing_rate for a in analyses if a.breathing_rate])
                    / len([a.breathing_rate for a in analyses if a.breathing_rate])
                    if analyses
                    else None
                ),
                "min_breathing_rate": (
                    min([a.breathing_rate for a in analyses if a.breathing_rate]) if analyses else None
                ),
                "max_breathing_rate": (
                    max([a.breathing_rate for a in analyses if a.breathing_rate]) if analyses else None
                ),
            },
        }
...
class AlertService:
    ...
    def get_device_alerts(
        db: Session, device_id: str, user_id: uuid.UUID, include_acknowledged: bool = False, limit: int = 50
    ) -> List[Alert]:
        """デバイスのアラート一覧取得"""

        query = db.query(Alert).join(Device).filter(and_(Device.device_id == device_id, Device.owner_id == user_id))
    ...
    @staticmethod
    async def acknowledge_alert(db: Session, alert_id: uuid.UUID, user_id: uuid.UUID) -> Optional[Alert]:
        """アラート確認"""

        alert = db.query(Alert).join(Device).filter(and_(Alert.id == alert_id, Device.owner_id == user_id)).first()
