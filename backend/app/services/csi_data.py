...
from app.models.csi_data import Session as DataSession
...
class CSIDataService:
    ...
    async def upload_csi_data(
        db: Session, device_id: str, file_data: bytes, upload_info: CSIDataUpload, user_id: uuid.UUID
    ) -> CSIData:
        """CSIデータアップロード処理"""
        ...
        try:
            ...
            try:
                task_id = await task_queue.enqueue_task(
                    ...
                    {"csi_data_id": str(csi_data.id), "analysis_params": {}},
    ...
    def get_csi_data_list(
        db: Session, user_id: uuid.UUID, filters: CSIDataFilter = None, page: int = 1, page_size: int = 20
    ...
    @staticmethod
    def get_csi_data_by_id(db: Session, csi_data_id: uuid.UUID, user_id: uuid.UUID) -> Optional[CSIData]:
        """特定CSIデータ取得"""
        return (
    ...
    @staticmethod
    async def delete_csi_data(db: Session, csi_data_id: uuid.UUID, user_id: uuid.UUID) -> bool:
        """CSIデータ削除"""
    ...
    @staticmethod
    def get_device_csi_stats(db: Session, device_id: str, user_id: uuid.UUID, days: int = 7) -> Dict[str, Any]:
        """デバイスのCSI統計情報取得"""
        device = db.query(Device).filter(and_(Device.device_id == device_id, Device.owner_id == user_id)).first()
        ...
        # 基本統計
        total_data = (
            db.query(CSIData).filter(and_(CSIData.device_id == device.id, CSIData.created_at >= start_date)).count()
        )

        status_stats = (
            db.query(CSIData.status, func.count(CSIData.id).label("count"))
            .filter(and_(CSIData.device_id == device.id, CSIData.created_at >= start_date))
            .group_by(CSIData.status)
            .all()
        )

        # セッション統計
        total_sessions = (
            db.query(DataSession)
            .filter(and_(DataSession.device_id == device.id, DataSession.created_at >= start_date))
            .count()
        )
        ...
        return {
            ...
            "last_upload": db.query(func.max(CSIData.created_at)).filter(CSIData.device_id == device.id).scalar(),
        }
...
class SessionService:
    ...
    @staticmethod
    async def create_session(db: Session, session_data: SessionCreate, user_id: uuid.UUID) -> DataSession:
        """セッション作成"""
        # デバイス存在確認
        device = (
            db.query(Device)
            .filter(and_(Device.device_id == session_data.device_id, Device.owner_id == user_id))
            .first()
        )
    ...
    @staticmethod
    async def end_session(db: Session, session_id: uuid.UUID, user_id: uuid.UUID) -> Optional[DataSession]:
        """セッション終了"""
        session = (
            db.query(DataSession)
            .join(Device)
            .filter(and_(DataSession.id == session_id, Device.owner_id == user_id))
            .first()
        )
    ...
    @staticmethod
    def get_device_sessions(db: Session, device_id: str, user_id: uuid.UUID, limit: int = 10) -> List[DataSession]:
        """デバイスのセッション一覧取得"""
        return (
            db.query(DataSession)
            .join(Device)
