"""
ZKP証明生成サービス（ウェーブレット変換版）

使用回路: csi_wavelet_similarity.circom (WaveletBreathingCheck)
  - 秘密入力: referenceMatrix[31][245], candidateMatrix[31][245]
             ※ Python側でCWT対数周波数軸を線形31点に再ビン化・整数スケール済み
...
CIRCUIT_NAME = "csi_wavelet_similarity"


class ZKPWaveletService:
    """ウェーブレット変換用ZKP証明生成サービス"""
    ...
    def _check_setup(self) -> None:
        ...
        if not (wasm.exists() and zkey.exists()):
            if self.auto_compile:
                logger.warning("Wavelet ZKP circuit files not found. Starting auto-compilation...")
                try:
                    ...
                    logger.warning(
                        "Please manually run:\n"
                        "  cd zkp && npm run compile:wavelet && npm run setup:wavelet"
                    )
            else:
                logger.warning(
                    "Wavelet ZKP circuit files not found. Please run:\n"
                    "  cd zkp && npm run compile:wavelet && npm run setup:wavelet"
                )
        else:
            logger.info("Wavelet ZKP circuit is ready")
    ...
    def _auto_compile_circuit(self) -> None:
        ...
        logger.info("Compiling wavelet circuit (this may take several minutes)...")
        result = subprocess.run(
            [
    ...
    def _run_trusted_setup(self) -> None:
        logger.warning("Starting Trusted Setup for Wavelet circuit (this may take 10-60 minutes)...")
        ...
        logger.info("Wavelet Trusted Setup completed")
        ...
        try:
            ...
            if not wasm.exists() or not zkey.exists():
                raise FileNotFoundError(
                    "Wavelet ZKP circuit files not found.\n"
                    "Please run: cd zkp && npm run compile:wavelet && npm run setup:wavelet"
                )
            ...
            logger.info(
                f"[Wavelet] Generating ZKP proof: "
            ...
            logger.info(
                f"[Wavelet] ZKP proof done in {time.time() - total_start:.3f}s "
            ...
            logger.error(f"[Wavelet] ZKP proof generation failed: {e}", exc_info=True)
            raise RuntimeError(f"Wavelet ZKP proof generation failed: {e}")
    ...
    async def verify_proof(
        ...
        try:
            ...
            logger.error(f"[Wavelet] Proof verification failed: {e}", exc_info=True)
            raise RuntimeError(f"Wavelet proof verification failed: {e}")
    ...
    async def _generate_witness(self, input_data: Dict[str, Any]) -> str:
        ...
        input_file = self.temp_dir / f"wavelet_input_{uuid.uuid4()}.json"
        witness_file = self.temp_dir / f"wavelet_witness_{uuid.uuid4()}.wtns"
        ...
        if proc.returncode != 0:
            ...
            raise RuntimeError(f"Wavelet witness generation failed: returncode={proc.returncode}\nstdout: {stdout_msg}\nstderr: {stderr_msg}")
    ...
    async def _generate_groth16_proof(self, witness_file: str) -> Tuple[Dict[str, Any], List[str]]:
        ...
        proof_file = self.temp_dir / f"wavelet_proof_{uuid.uuid4()}.json"
        public_file = self.temp_dir / f"wavelet_public_{uuid.uuid4()}.json"

        if not zkey.exists():
            raise RuntimeError(f"zkey file not found: {zkey}")

        if not Path(witness_file).exists():
            raise RuntimeError(f"Wavelet witness file not found: {witness_file}")
        ...
        if proc.returncode != 0:
            stdout_msg = stdout.decode(errors="replace") if stdout else ""
            stderr_msg = stderr.decode(errors="replace") if stderr else ""
            raise RuntimeError(f"Wavelet proof generation failed: returncode={proc.returncode}\nstdout: {stdout_msg}\nstderr: {stderr_msg}")
