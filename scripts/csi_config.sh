...
# ============================================================
SERVER_URL="${CSI_SERVER_URL:-http://192.168.101.16:8000}"
...
upload_csi() {
    ...
    file_size_bytes=$(wc -c < "$csi_file")
    file_size_human=$(du -h "$csi_file" | cut -f1)
    echo "[Upload] File size : ${file_size_human} (${file_size_bytes} bytes)"
    echo "[Upload] Uploading to ${upload_url} ..."

    # Cloudflare 無料プランは 100MB 上限のため警告
    if [[ "$file_size_bytes" -gt 104857600 ]]; then
        echo "[Upload] WARNING: ファイルが 100MB を超えています。Cloudflare 経由では 413 エラーになります。"
        echo "[Upload]          CSI_BASE_DURATION を短くするか、SSH 経由で直接アップロードしてください。"
        return 1
    fi
    ...
    if [[ "$http_status" -ge 200 && "$http_status" -lt 300 ]]; then
        ...
        return 0
    elif [[ "$http_status" -eq 413 ]]; then
        echo "ERROR: Upload failed. HTTP 413 (Payload Too Large)"
        echo "       ファイルサイズ (${file_size_human}) がサーバー上限を超えています。"
        return 1
    else
