...
ZKPのオンチェーン検証には、回路から生成されたVerifierコントラクトが必要です。
`snarkjs zkey export solidityverifier` で生成したVerifierをデプロイし、
`backend/contracts/build/FullSimilarityVerifier.json` にABIを配置してください。
デプロイ後は `ZKPROOF_VERIFIER_CONTRACT_ADDRESS` を設定します。
...
# ZKP検証コントラクトアドレス（回路ごとに生成・デプロイ）
ZKPROOF_VERIFIER_CONTRACT_ADDRESS=0x5678...
...
オンチェーン検証は Verifier コントラクト（`FullSimilarityVerifier.json`）を使用します。
証明検証は `/api/blockchain/verify-proof-on-chain` に統一しています。
