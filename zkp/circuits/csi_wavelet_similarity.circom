...
/**
 * CSI呼吸モニタリング正常判定回路（ウェーブレット変換版）
 *
 * 設計方針：
 *   秘密入力：ウェーブレット変換適用済みCSIデータ（0.0～0.6Hz、0.02Hz刻み、31点）
 *             ※ Python側でCWTの対数周波数軸を線形31点に再ビン化したもの
...
template WaveletSubcarrierDotNorms(N) {
    signal input ref[N];
    signal input cand[N];
    signal output dot;
...
template WaveletBreathingCheck(
    NUM_FREQ_POINTS,
    NUM_SUBCARRIERS,
    NORMAL_LOW_BIN,
    NORMAL_HIGH_BIN
    ...
    for (var sc = 0; sc < NUM_SUBCARRIERS; sc++) {
        dn[sc] = WaveletSubcarrierDotNorms(NUM_FREQ_POINTS);
 ...
 */
component main = WaveletBreathingCheck(31, 245, 5, 25);
